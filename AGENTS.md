...
- Deep links (SPA): workspace `/workspaces/{id}`; project `/projects/{id}`; task `/projects/{id}?tab=tasks&task={key}[&section=comments|attachments|dependencies|time|activity]`. `resources/js/utils/notifications.js` `notificationHref(data, type)` is **type-aware** — `task.commented` → `section=comments`, `task.work_logged` → `section=time`; falls back to `/workspaces/{id}` when only `workspace_id` is present. `ProjectDetail` keeps the query **in the URL** (`?task` + `?section`, refresh/direct-tab safe), forces board view for deep links, and auto-opens the drawer via `openedDeepTaskRef`; `openTask(task, section)` passes the section down; `TaskDetail` accepts an `initialSection` prop and forces that drawer sub-tab on `[task]` change (validated against details/comments/attachments/dependencies/time/activity). `ProtectedRoute` preserves `pathname + search` through the login redirect so an unauthenticated deep link resumes after login.
...
- Composer `laravel/reverb`; npm `laravel-echo` + `pusher-js` (+ `@dnd-kit/*` for the board). `BROADCAST_CONNECTION=reverb`.
